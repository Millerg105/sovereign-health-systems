    ...
    <PortfolioPage>
      <div className="flex h-full flex-col px-7 py-7 sm:px-9 sm:py-8 lg:px-12 lg:py-9">
        ...
        <div className="mt-5 grid flex-1 gap-3 lg:grid-cols-2 xl:grid-cols-3">
          {groups.map((group) => (
            <section key={group.id} className="glass-panel flex flex-col rounded-[24px] border-white/10 p-3">
              <div className="mb-3 flex items-center justify-between gap-3">
                <h3 className="font-body text-base font-semibold uppercase tracking-[0.06em] text-white">{group.label}</h3>
                <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-brand-cyan/80">Hero Frame</span>
              </div>
              <div className="portfolio-showcase-card min-h-[220px] flex-1">
              ...
              <div className="mt-3 flex items-center justify-between gap-2">
                ...
                {group.siteHref ? (
                  <Link
                    ...
                    className="inline-flex items-center rounded-full border border-brand-cyan/28 bg-brand-cyan/10 px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.14em] text-cyan-50 transition-all hover:-translate-y-px hover:border-brand-cyan/55 hover:bg-brand-cyan/15"
                  >
                    {group.siteLabel}
                  </Link>
                ) : (
                  <div className="inline-flex items-center rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.14em] text-brand-cyan/90">
                    {group.siteLabel}
                  </div>
                )}
              </div>
